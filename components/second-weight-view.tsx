    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        for (let i = 6; i < result.data.length; i++) {
          ...
          // Extract data from columns B, C, D, E, I, M, N, O, P, Q (indices 1, 2, 3, 4, 8, 12, 13, 14, 15, 16)
          const orderNumber = row[1] || ""
          const gateEntryNumber = row[2] || ""
          const customerName = row[3] || ""
          const truckNumber = row[4] || ""
          const wbSlipNo = row[8] || ""
          const supervisorName = row[12] || ""
          const remarks = row[13] || ""
          const columnO = row[14] || ""
          const columnP = row[15] || ""
          const finalWeight = row[16] || ""
          const planned4 = formatDateTime(row[14]) || ""
          ...
          if (orderNumber || gateEntryNumber || customerName || truckNumber) {
            ...
            // Pending condition: column O is not null and column P is null
            if (columnO && columnO.toString().trim() !== "" && (!columnP || columnP.toString().trim() === "")) {
              pending.push(entry)
            }
            // History condition: both column O and column P are not null
            else if (columnO && columnO.toString().trim() !== "" && columnP && columnP.toString().trim() !== "") {
              history.push(entry)
            }
          }
        }
        ...
        {activeTab === 'pending' && (
          <div className="bg-white rounded-lg shadow border">
            <div className="px-6 py-4 border-b">
              ...
              <p className="text-gray-600 text-sm">Vehicles waiting for final weighing</p>
            </div>
                  ...
                  {pendingData.length > 0 ? (
                    pendingData.map((entry, index) => (
                      <tr key={entry.gateEntryNumber || index} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{entry.planned4}</td>
        ...
        {activeTab === 'history' && (
          <div className="bg-white rounded-lg shadow border">
            <div className="px-6 py-4 border-b">
              ...
              <p className="text-gray-600 text-sm">Completed final weighing records</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border-b">
